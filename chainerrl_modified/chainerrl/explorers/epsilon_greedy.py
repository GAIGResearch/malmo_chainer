...
def select_action_epsilon_greedily(epsilon, random_action_func,
    ...
    if np.random.rand() < epsilon:
        print("random_action_func name = "+str(random_action_func))
        ra+=1
        print("EXPLORER: random actions="+str(ra)+", greedy actions="+str(ga))
        return random_action_func(), False
    else:
        print("greedy_action_func name = "+str(greedy_action_func))
        ga+=1
        print("EXPLORER: random actions="+str(ra)+", greedy actions="+str(ga))
...
class ConstantEpsilonGreedy(explorer.Explorer):
    ...
    def __init__(self, epsilon, random_action_func,
        ...
        print("explorer random_action_func parameter = "+str(random_action_func))
        self.epsilon = epsilon
        self.random_action_func = random_action_func
        self.logger = logger

    def select_action(self, t, greedy_action_func, action_value=None):
        print("select action self.random_action_funct = "+str(self.random_action_func))
...
class LinearDecayEpsilonGreedy(explorer.Explorer):
    ...
    def __init__(self, start_epsilon, end_epsilon,
        ...
        print("explorer random action function: "+self.random_action_func)
