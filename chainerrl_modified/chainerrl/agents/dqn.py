...
class DQN(agent.AttributeSavingMixin, agent.Agent):
    ...
    def act_and_train(self, state, reward):
        ...
        with chainer.using_config('train', False):
            with chainer.no_backprop_mode():
                print("DQN states = "+str(state))
                print("DQN self.xp = "+str(self.xp))
                action_value = self.model(
                    self.batch_states([state], self.xp, self.phi))
                print("DQN action_value.greedy_actions.data = "+str(action_value.greedy_actions.data))
                q = float(action_value.max.data)
                print("DQN q="+str(q))
        ...
        print("DQN action_value = "+str(action_value))
        print("DQN greedy_action = "+str(greedy_action))
        action = self.explorer.select_action(
            self.t, lambda: greedy_action, action_value=action_value)
        print("DQN action = "+str(action))
        self.t += 1
        ...
        print("DQN self.last_action = "+str(self.last_action))
